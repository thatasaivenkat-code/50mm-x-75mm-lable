...
# --- 1. PAGE CONFIG & LOGO FOLDER SETUP ---
st.set_page_config(page_title="🚀 వాయి వేగ Multi-Tool Pro", layout="wide")

LOGO_FOLDER = "logos"
if not os.path.exists(LOGO_FOLDER):
    os.makedirs(LOGO_FOLDER)

# --- 2. ULTRA PREMIUM & CLEAR UI THEME (CSS) ---
st.markdown("""
<style>
...
html, body, [class*="stApp"] {
    ...
    background-color: #f8fafc !important; /* Clean Cool Grey/White */
}

/* Sidebar Styling - Deep Luxury Indigo */
...
[data-testid="stSidebar"] .stRadio j {
    color: #f1f5f9 !important;
}
...
/* Streamlit Native Components Overrides for High Visibility */
.stButton>button {
...
# --- 3. ADVANCED LABEL GENERATION FUNCTION ---
...
# ===============================================
with st.sidebar:
    st.markdown('<div style="text-align:center; padding:10px 0 20px 0;"><h2 style="color:#3b82f6; font-weight:800; margin:0; font-size:1.6rem;">⚡ VAYU VEGA</h2><p style="color:#94a3b8; font-size:0.8rem; margin:0;">Enterprise Hub v3.5</p></div>', unsafe_allow_html=True)
    ...
    choice = st.radio("Navigation Options", 
                     ["🏠 Dashboard", "🕹️ Advanced Controls", "📦 Barcode Pro", "📊 PDF→Excel"],
                     index=0,
...
if choice == "🏠 Dashboard":
    ...
    st.markdown('<p class="subtitle">Unified Logistics & Business Intelligence Automation Hub</p>', unsafe_allow_html=True)

    # Clean UI Grid Layout for Dashboard Home
    st.markdown("""
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div class="feature-card">
            ...
            <div>
                ...
                <div class="card-desc">Dynamic Joystick tuning with Multi-format thermal sheet mapping.</div>
            </div>
        </div>
        <div class="feature-card">
            ...
            <div>
                ...
                <div class="card-desc">3-inch standard bulk tracking barcode labels for A4 print.</div>
            </div>
        </div>
        <div class="feature-card">
            <span class="feature-icon">📊</span>
            <div>
                ...
                <div class="card-desc">AI-powered parsing extraction supporting DTDC and Delhivery formats.</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background: #eff6ff; padding: 20px; border-radius: 12px; border: 1px solid #bfdbfe; text-align: center; max-width: 700px; margin: 40px auto 0 auto;">
        <h4 style="color:#1e40af; margin-top:0; font-weight:700;">시스템 준비 완료 (System Ready)</h4>
        <p style="color:#1e3a8a; margin-bottom:0; font-size:0.95rem;">అన్ని మోడ్యూల్స్ అప్‌గ్రేడ్ చేయబడ్డాయి బ్రో. ఎడమవైపు ఉన్న <b>Sidebar Panel</b> నుండి మీకు కావలసిన ఆప్షన్‌ను ఎంచుకోండి!</p>
    </div>
    """, unsafe_allow_html=True)

# ===============================================
# 🕹️ MODULE 1: ADVANCED CONTROLS
# ===============================================
    ...
    uploaded_logo = st.sidebar.file_uploader("Upload External Logo", type=['png', 'jpg'])
    ...
    with tab1:
        ...
        with c_form:
            with st.form("single_form"):
                ...
                with c1:
                    ...
                    p_name, p_val = st.text_input("Declared Product Type", "General"), st.text_input("Assigned Commercial Value", "1000")
                    l_date = st.text_input("Label Date Stamp", datetime.now().strftime("%d-%m-%Y"))
                    wt, t_amt, ref_no = st.text_input("Physical Weight (KG)", "0.5"), st.text_input("Invoice Total Amount", "1100"), st.text_input("Customer Ref Number", "REF-01")
                    mode_opt = st.selectbox("Logistics Channel Mode", ["Surface", "Express"])
                    risk_opt = st.selectbox("Consignment Risk Cover", ["Carrier", "No Risk"])
                with c2:
                    t_name, t_phone, t_pin = st.text_input("Consignee (To Name)", "CHINNA RAO"), st.text_input("Consignee Contact Mobile", "9876543210"), st.text_input("Destination Pincode", "500001")
                    ...
                    f_name, f_phone = st.text_input("Shipper (From Name)", "Vayu Vega Hub"), st.text_input("Shipper Contact Mobile", "8888888888")
                    f_addr = st.text_area("Full Origin Address Coordinates", "Hub Main Center, Vijayawada")
                submit_single = st.form_submit_button("🔥 Refresh Design Grid View")
        ...
        with c_view:
            ...
            st.download_button(f"📥 Export Current PDF File", current_pdf, f"{awb_no}_{ref_no}.pdf", use_container_width=True)
    ...
    with tab2:
        ...
        st.markdown("---")
        uploaded_file = st.file_uploader("Upload Batch Operations Matrix File", type=['csv', 'xlsx'])
        ...
        if uploaded_file:
            ...
            if st.button(f"🚀 Compile System Engine for {len(selected_rows)} Manifest Items", use_container_width=True):
                if len(selected_rows) == 0:
                    ...
                    with col_d1:
                        st.download_button(f"📥 Download Independent Files (ZIP)", zip_buffer.getvalue(), "Selected_Labels.zip", use_container_width=True)
                    with col_d2:
                        st.download_button(f"📥 Download Combined Continuous Grid (PDF)", bulk_pdf_buffer.getvalue(), "Selected_Labels_Combined.pdf", use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

# ===============================================
# 📦 MODULE 2: BARCODE PRO
# ===============================================
    ...
    with colb1:
        numbers_input = st.text_area("📝 Input Barcode Sequential IDs (Line Separated):", 
                                   height=200, 
    ...
    if st.button("🖨️ Compile A4 Standard Print Grid Sheet", use_container_width=True):
        if numbers_input.strip() and company_name.strip():
            ...
            try:
                ...
                st.download_button("📥 Save A4 Composite Print Sheet PDF", pdf_buffer.getvalue(), 
                                 f"{company_name.replace(' ', '_')}_A4_Labels.pdf", use_container_width=True)
...
# ===============================================
# 📊 MODULE 3: PDF TO EXCEL DATA EXTRACTOR
# ===============================================
    ...
    pdf_files = st.file_uploader("📄 Drop Digital PDF Files Directly", type=['pdf'], accept_multiple_files=True)
    ...
    if pdf_files and st.button("🔄 Execute Extraction Pipeline Matrix", use_container_width=True):
        ...
        if all_extracted_data:
            ...
            st.download_button("📥 Save Formatted Master Spreadsheet Report", 
